...
def main(args):
    ...
    optimizer = optim.Adam(
        [
            {"params": model.egcnblock.parameters(), "lr": 3e-2},  # 0.3
...
def train(args, model, train_loader, optimizer, epoch, training_step, writer):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        if training_step == 1 or training_step == 2:  # 仅编码，无解码，输出要与输入接近
            model_input = obs_traj_rel
            # print("model_input[-args.pred_len:]：",model_input[-args.pred_len:].shape)  # [8,n,2]
            ...
            for _ in range(args.best_k):
                ...
                l2_loss_rel.append(
                    l2_loss(
                        pred_traj_fake_rel,
                        model_input[-args.pred_len:],  # 最终输出要与输入的最后pred_len个时刻接近
                        loss_mask,
                        mode="raw",
                    )
                )
